# Polish Property Transaction Data
...
Run the transaction pipeline directly (defaults to Mokotów apartments, saves to `lokale_mokotow.duckdb` and `lokale_mokotow.parquet`):
...
`data_plot.png` — 2×2 summary plot generated by `plot_transactions()`:

![Transaction summary plot](data_plot.png)

`gdf.head(3).T` from `lokale_mokotow.parquet`:
